"""Template for generating per-problem spec.py files."""
from __future__ import annotations

# The canonical boilerplate is stored verbatim. Four placeholder tokens are
# substituted via str.replace (NOT str.format) to avoid escaping all the
# literal { } braces that appear in the boilerplate (f-strings, dicts, etc.).
#
# Placeholders chosen to be unique strings unlikely to appear elsewhere:
#   @@CFG_MODULE@@   -> _cobench_cfg_{slug}
#   @@SPEC_NAME@@    -> cobench_{slug}
#   @@STARTER_CODE@@ -> repr(starter_code)
#   @@DIRECTION@@    -> repr(direction)

_TEMPLATE = '''\
"""ISTH spec for CO-Bench task: '@@SPEC_NAME@@' (auto-generated)."""
from __future__ import annotations
import inspect
import math
import sys
...
_mod_spec = importlib.util.spec_from_file_location("@@CFG_MODULE@@", _CONFIG_PATH)
_cfg = importlib.util.module_from_spec(_mod_spec)
_mod_spec.loader.exec_module(_cfg)


_DIRECTION = @@DIRECTION@@   # \'min\' or \'max\' (see tools/build_cobench_specs.py)
...
def render_spec(
    ...
    """Return a complete spec.py source string with the four tokens substituted."""
    if name is None:
        name = f"cobench_{slug}"

    result = _TEMPLATE
    result = result.replace("@@CFG_MODULE@@", f"_cobench_cfg_{slug}")
    # Replace both occurrences: the importlib call AND inside the class name attr
    result = result.replace("cobench_@@SPEC_NAME@@", name)
    result = result.replace("@@SPEC_NAME@@", slug)
    result = result.replace("@@STARTER_CODE@@", repr(starter_code))
    result = result.replace("@@DIRECTION@@", repr(direction))
    return result
