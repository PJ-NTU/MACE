...
def review(llm_client, role: str, problem_description: str, artifact_code: str,
    ...
    prompt = (
        ...
        f"  - input validation / error handling / robustness — raising on malformed,\n"
        f"    short, or extra input is perfectly acceptable; do NOT ask for graceful handling;\n"
        f"  - 'could handle X', 'could be more robust', 'should also support' suggestions;\n"
        f"  - 'could be more explicit' / 'should also mention' suggestions.\n"
        f"It already parses the real sample without error (a machine check confirmed this), "
        f"so only flag a defect in WHAT it produces, never in how defensively it is written.\n"
