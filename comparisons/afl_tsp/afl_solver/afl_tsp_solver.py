...
if __name__ == "__main__":
    ...
    parser.add_argument('--time-limit', type=float, default=None, help='Wall-clock time budget in seconds; overrides iteration count when set')
    ...
    # When --time-limit is set, run until the wall-clock budget is exhausted
    # (large iteration ceiling so time is the stopping criterion). Only the
    # stopping rule is changed; the destroy-repair search itself is unmodified.
    _start = time.time()
    _iter_ceiling = iteration if time_limit is None else 10**9
    for step in range(_iter_ceiling):
        if time_limit is not None and (time.time() - _start) >= time_limit:
            break
        ...
        if current_cost <= best_cost:
            ...
            _denom = max(1, (iteration - step + 1)) if time_limit is None else 100
            threshold = math.exp(min(700, -(current_cost - best_cost) * iteration * 10 / _denom))
            if p > threshold:
